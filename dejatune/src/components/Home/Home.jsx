...
const Home = () => {
  ...
  useEffect(() => {
    const unsub = auth.onAuthStateChanged(u => {
      if (!u) navigate('/');  // not logged in → back to login
      else setUser(u);
    });
    return unsub;
  }, [navigate]);

  // 2️⃣ Real-time Firestore: RecentSongs & SavedSongs
  useEffect(() => {
    if (!user) return;
  ...
  const identifySong = async () => {
    ...
    try {
      ...
      for (let i = 0; i < 10 && !found; i++) {
        const raw = await (await model.generateContent(buildFormPrompt())).response.text();
        ...
        if (phrases.some(p => verse.toLowerCase().includes(p.toLowerCase()))) {
          // bold user fragments
...
${history}
  ...
  const handleSendInitial = () => {
    ...
    setConversation(c => [...c, { speaker:'user', text: inputValue.trim() }]);
    setInputValue('');
    askClarifyingQuestion();
  };
  ...
  const clearAll = () => {
    setShowForm(false);
    setShowChat(false);
    setPhrases(['']); setGenre(''); setYear(''); setTone(''); setVibe('');
    setConversation([]); setCurrentQuestion(''); setOptions([]);
            ...
            <section>
              ...
              {phrases.map((p,i)=>(
                <input
                  ...
                  type="text"
                  value={p}
                  onChange={e=>setPhrases(u=>{ u[i]=e.target.value; return [...u]; })}
                  placeholder={`Fragment ${i+1}`}
                />
              ))}
              <button onClick={()=>setPhrases(u=>[...u,''])}>+ Add Fragment</button>
            </section>
            <section>
              <h3>Filters</h3>
              <div className="filters-grid">
                ...
                <input placeholder="Mood"  value={tone}  onChange={e=>setTone(e.target.value)}/>
        ...
        {showChat && !result && (
          <div className="chat-container">
            {conversation.map((m,i) => (
              <div key={i} className={`chat-bubble ${m.speaker==='user'?'user':'bot'}`}>
                {m.text}
              </div>
            ))}
